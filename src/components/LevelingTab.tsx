...
export default function LevelingTab({ xp, avatar }: { xp: number; avatar?: AvatarConfig }) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-baseline justify-between">
              <span className="text-lg font-bold text-slate-900">
                {level === 0
                  ? "Débutant"
                  : (ACCESSORY_UNLOCKS.find((u) => u.level === level)?.label ?? `Niveau ${level}`)}
              </span>
              <span className="text-xs text-slate-400">
                {xp} XP{!isMax && ` · encore ${toNext} XP`}
              </span>
            </div>
            ...
            <div className="mt-1 flex justify-between text-[10px] text-slate-400">
              <span>Niv. {level}</span>
              {!isMax && <span>Niv. {level + 1} — {xpForLevel(level + 1)} XP</span>}
              {isMax && <span>Niveau max atteint 👑</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Accessories grid */}
      <div>
        <h2 className="label mb-3">Accessoires à débloquer</h2>
              ...
              >
                ...
                <div>
                  <div className={cx("text-xs font-semibold", unlocked ? "text-slate-900" : "text-slate-400")}>
                    {unlocked ? `${unlock.emoji} ${unlock.label}` : `Niv. ${unlock.level}`}
                  </div>
                  <div className="mt-0.5 text-[10px] text-slate-400">
                    {unlocked ? unlock.description : `Encore ${xpForLevel(unlock.level) - xp} XP`}
                  </div>
                </div>
              </div>
            );

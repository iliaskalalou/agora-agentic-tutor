...
- **A personal learner profile** — name, a custom avatar (a buddy that follows them everywhere),
  and interests. Exercises are then **AI-generated as adventure scenes** themed to that avatar and
  those interests (math = a locked gate to open, geography = a path to find, …). The narrative is
  generated by a **local, keyless LLM (Ollama)**; the answer choices and the correct answer stay
  anchored to the verified knowledge base, so a small local model can never produce a wrong key.
...
![Create your learner — profile & avatar](docs/screenshots/04-profile.png)
...
- **Provider-agnostic LLM via `fetch`** (no SDK) — Anthropic, OpenAI, or a **local keyless Ollama
  model**; it degrades to the deterministic engine on any error, so a run never fails on a network
  hiccup. No API keys are ever stored in the repo.
